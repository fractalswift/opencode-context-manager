...
export function getChangedFiles(oldCommit, newCommit = 'HEAD', dir = '.') {
  ...
  try {
    // Use --name-status to get both filename and change type
    // -M to detect renames
...
export function checkExportChanges(filePath, oldCommit, newCommit = 'HEAD', dir = '.') {
  try {
    ...
    const exportChanges = diff
      .split('\n')
      .filter(line => {
        // Check for +/- lines that contain 'export'
...
export function categorizeFile(filePath) {
  ...
  // Components
  if (path.match(/components\/.*\.(tsx|jsx|vue|svelte)$/)) {
    return 'component';
  }
  
  // Hooks (React/Vue)
  if (path.match(/(hooks|composables)\/use.*\.(ts|tsx|js|jsx)$/)) {
    return 'hook';
  }
  
  // Services
  if (path.match(/services\/.*\.(ts|tsx|js|jsx)$/)) {
    return 'service';
  }
  
  // API/Routes
  if (path.match(/(api|routes)\/.*\.(ts|tsx|js|jsx)$/)) {
    return 'api';
  }
  
  // Utilities
  if (path.match(/(utils|helpers|lib)\/.*\.(ts|tsx|js|jsx)$/)) {
    return 'utility';
  }
  
  // Types
  if (path.match(/types\/.*\.(ts|d\.ts)$/) || path.endsWith('.d.ts')) {
    return 'type';
  }
